...
import { Btn, Card, Badge, avatarColor } from "../components/ui";
...
export default function Dashboard({ go, stats, recentRows }: DashboardProps) {
    ...
    <div className="fade-page">
      <div className="page-head">
        ...
        <div className="ph-actions">
          <Btn variant="ghost" size="sm" icon="Refresh" onClick={() => window.location.reload()}>새로고침</Btn>
      ...
      <div className="page-scroll">
        <div className="stat-grid">
          {[
            { ico: "Database" as const, val: (stats.total ?? 0).toLocaleString(), label: "누적 수집 연락처", delta: "+12.4%", up: true },
            { ico: "Zap" as const, val: (stats.today ?? 0).toLocaleString(), label: "오늘 수집", delta: "+38", up: true },
            { ico: "Target" as const, val: stats.total > 0 ? `${Math.round(((stats.verified || 0) / stats.total) * 100)}%` : "0%", label: "유효 연락처 비율", delta: "+2.1%", up: true },
            { ico: "Shield" as const, val: (stats.dupRemoved || 0).toLocaleString(), label: "중복 제거", delta: "-6", up: false },
          ].map((s) => (
        ...
        <div style={{ marginTop: 14 }}>
          <Card title="최근 활동" sub="방금 수집된 연락처"
            ...
            <div style={{ marginTop: -4 }}>
              {recentRows.length === 0 ? (
                <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-4)' }}>최근 수집된 데이터가 없습니다.</div>
              ) : (
                recentRows.map((co) => (
                  <div key={co.id} className="activity-row">
                    <span className="cell-avatar" style={{ background: avatarColor(co.company_name || '회사'), width: 32, height: 32 }}>
                      {co.company_name?.[0] || '?'}
                    </span>
                    <div className="act-main">
                      <div className="act-title">{co.company_name}</div>
                      <div className="act-meta">
                        {co.keyman_candidates?.[0]?.name || '담당자 미상'} {co.keyman_candidates?.[0]?.title || ''} · {co.region_sido || '지역 미상'} · {co.industry || '업종 미상'}
                      </div>
                    </div>
                    {co.phone_status === 'verified' && <Badge tone="accent" dot>검증됨</Badge>}
                    <span className="act-time">{co.collected_at || co.created_at}</span>
                  </div>
                ))
              )}
            </div>
